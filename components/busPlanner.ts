 ...
 * ?òÈ????busPlanner.py (Refactored Version)
...
// ??¨Ë∫´?Ó©ìÔÖ≥??JSON ?ºÓ???øÔ?Ó°ìÊ????ØÓ???ùÂ?Ëø§Ëù£ÁÆ??øÔ???// ?‰∫ôÓØ≠ Expo ?ùÂì®??¢Ô??°ÈùΩÓ≥ã¬ÄÓ©??ºÓ???ùÔ????î‰??†Ó???Bundle ?≠Ê???óÔó∫?????ÂØßÓ? expo-file-system ?ùÔ??
...
export interface BusInfo {
  ...
  stopCount: number;
  estimatedDuration?: number; // ??çÊëØ????Óø??óÔ???ÓßÄ?
...
export class BusPlannerService {
  ...
  constructor() {
    // ??React Native ?ùÂì®?JSON import ????á‰∫¶??óÔóº????Óµ??á‰???™Ô??
    // ?øÓµ§?????û‰∫¶Ê≥????Ó©???
  ...
  // --- Public API Methods (?ÜÓ∞™? Vercel ??áÓØ±?ªÁ????ìÓ??? ---

  /**
   * ?Ó°???? SID ??íÓØµ?????
   * @param sid ?°Ó?? ID
   * @returns ????øÔ????undefined
   */
  ...
  public async fetchBusesAtSid(sid: string): Promise<any[]> {
    ...
    if (!slid) {
      console.warn(`[BusPlanner] SID ${sid} ?ùÓ???†Ô????SLID`);
      return [];
    }
  ...
  private findStaticRoutes(startName: string, endName: string): StaticRouteMatch[] {
    ...
    for (const route of this.routeDb) {
        ...
        // 1. ??âÔäæ?ùË???ùÂ????•Ê≥µ??Ñ¬Ä?ΩÁµ≤?∫Óµ§??îÊ?????øÔ??≠Ë?Ôº?
        const startIndices = stops
            .map((sid, idx) => startSids.has(sid) ? idx : -1)
            .filter(i => i !== -1);
            
        // 2. ??âÔäæ?ùË???ùÂ????•Ê≥µ??Ñ¬Ä???∫Óµ§??îÊ?????øÔ??≠Ë?Ôº?
        ...
        // 3. ?????¥Êë© (??Python _match_single_route ?†Ô??)
        for (const sIdx of startIndices) {
            // ??âÔ??∞ËÑ∞Áµ≤Êö∫Óµ??∫Ô????È§àÓï≠??ù¬Ä?????
            ...
            if (firstValidEnd !== undefined) {
                ...
                // ?ΩÊ†ºËø§Â?Ó´≤Â???break?óÔóº?±Ëù•?ªÁ??‰π??ù¬Ä??startIndices
                // ?òÔ???óÓ´±??ùË????ΩÊ¥µ 5 ?°Ó????20 ?°Ó??çËù¨????ªÊ??ûÊ∞¨?????µ¬Ä?≥Ô???™Ô?????úÓü°??ùÔ????
            }
        }
    }
    return candidates;
  ...
  /**
   * ?ÂØûÊ¥ª????¢Ô???û‰∫§???°Ëî•?ÊΩ? (?Ö‚ä•??Python ??asyncio + batch logic)
   */
  ...
  private async fetchRealtimeBySlid(slid: string, repSid: string): Promise<any[]> {
    ...
    try {
        ...
        // ??? HTML ?µÂÜΩÓπµÊí±ÁÆ? rid ?†Ô??±Èäµ????ØÓ??Â≠??àÔã¨???
        $('tr').each((_, row) => {
            const $row = $(row);
            const cols = $row.find('td');
            
            // ???¨Óºæ?∑Ê??3 ????øÔö°??ùË???Óº???∏¬ÄÓººÓ???©¬ÄÓº???
            ...
            for (const item of resJson.Stop) {
                ...
                if (routeMap[jDynId]) {
                    ...
                    buses.push({
                        ...
                        rid: info.rid,
                        sid: repSid,
                        direction: info.direction, // ?Ó∫ûÔÖØ?Â≠??àÔã¨?
                        time_text: timeText,
        ...
        for (const k in routeMap) {
            buses.push({
                ...
                rid: routeMap[k].rid,
                sid: repSid,
                direction: routeMap[k].direction, // ?Ó∫ûÔÖØ?Â≠??àÔã¨?
                time_text: '\u672a\u767c\u8eca',
  ...
  public async plan(startName: string, endName: string): Promise<BusInfo[]> {
    ...
    // ??âÔäæ??????¨ÓººÓ∑ÅÈñ∞Ôº? SLID (?Èß?)
    const slidMap = new Map<string, string>(); // slid -> repSid (???ùSID)
    ...
    // ?±Á???πÊ??Ó∏ÇÓ???á‚Ä? SLID -> Array of RealtimeData
    const realtimeLookup: Record<string, any[]> = {};
    allRealtimeBuses.forEach(b => {
        // ?Ó∫†Óæ≠ realtime data ??? rid ??time?óÔóª?????¨ÓºΩÓ∑????ÖÁ?Ó°??∏¬Ä?SLID
        // ?Ó©ñ„??îÔ???trick?óÓ´±???´ÓØ≠ fetchRealtimeBySlid ?Ü‚à™??‰π? sid?óÔó∫???sid -> slid
    ...
    for (const route of matchedRoutes) {
        ...
        finalBuses.push({
            ...
            estimatedDuration: Math.ceil((pathStops.length - 1) * 2 + 1), // ?°Â???óÓ´±??????+?∫Êäµ?1???
  ...
  public async getStopArrivals(stopName: string): Promise<any[]> {
    ...
    if (!this.stopDb) {
      ...
      // ?‰∫§Ó???constructor ????á‰∫•???JSON?óÔóæ?Ó©ñ„???¨Ëï≠?‰º??‰∫§Ó??Óµ??á‰???????? init
    }
    ...
    // 3. ??âÔäæ?ùÔ???¥Ôã´? SLID (Stop Location ID) ?û‰ªøÓº????¥Ôã¨???
    // ??¥Êë©?óÓ´∞??ù¬Ä?????óÓèª?Ë≥??≠Ó´∞?????(SID)?óÔóπ??∞ÔÅµ??´Óèª?Ë≥¢ÔÖª?≠ÊÄ??ù¬Ä????????(SLID)
    ...
    // 4. ?ÂØûÊ¥ª?ùË•ø???? (?øËºª??Ôº???batchProcess ?àÓ???
    const tasks = Array.from(slidMap.entries()).map(([slid, sid]) => ({ slid, sid }));
